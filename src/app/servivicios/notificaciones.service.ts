...
export class NotificacionesService {
  ...
  constructor(private spinner: NgxSpinnerService, private toastControl: ToastrService) { }
  ...
  {
    const toast = await this.toastControl.warning("", mensaje, {
      timeOut: 3000, 
    });
  }

  async toastNotificationInfo(mensaje: string) {
    const toast = await this.toastControl.info("", mensaje, {
      timeOut: 3000,
  ...
  async toastNotificationSuccess(mensaje: string) {
    const toast = await this.toastControl.success("", mensaje, {
      timeOut: 3000,
    });
  }
  async toastNotificationError(mensaje: string) {
    const toast = await this.toastControl.error("", mensaje, {
      timeOut: 3000,
    });
  }
